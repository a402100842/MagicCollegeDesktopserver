...
public abstract class GenericPool<T> {
	...
	public synchronized T obtainPoolItem() {
		...
		if(this.mAvailableItems.size() > 0) {
			...
			System.out.println(this.getClass().getName() + "<" + item.getClass().getSimpleName() +"> was exhausted, with " + this.mUnrecycledCount + " item not yet recycled. Allocated " + this.mGrowth + " more.");
		}
	...
	public synchronized void recyclePoolItem(final T pItem) {
		...
		if(this.mUnrecycledCount < 0) {
			System.out.println("More items recycled than obtained!");
		}
	}
